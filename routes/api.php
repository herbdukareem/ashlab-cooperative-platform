...
use App\Http\Controllers\Api\AccountingPostingRuleController;
...
Route::middleware('auth:sanctum')->group(function (): void {
    ...
    Route::middleware('tenant')->group(function (): void {
        ...
        Route::apiResource('ledger-accounts', LedgerAccountController::class)->middleware('permission:accounting.configure');
        Route::get('accounting-periods', [AccountingPeriodController::class, 'index'])->middleware('permission:accounting.view');
        Route::post('accounting-periods', [AccountingPeriodController::class, 'store'])->middleware('permission:accounting.periods.manage');
        Route::patch('accounting-periods/{period}/close', [AccountingPeriodController::class, 'close'])->middleware('permission:accounting.periods.manage');
        Route::patch('accounting-periods/{period}/reopen', [AccountingPeriodController::class, 'reopen'])->middleware('permission:accounting.periods.manage');
        Route::get('journals', [JournalController::class, 'index'])->middleware('permission:accounting.view');
        Route::post('journals', [JournalController::class, 'store'])->middleware('permission:accounting.post');
        Route::get('journals/{journal}', [JournalController::class, 'show'])->middleware('permission:accounting.view');
        Route::post('journals/{journal}/reverse', [JournalController::class, 'reverse'])->middleware('permission:accounting.reverse');
        Route::get('accounting-posting-rules', [AccountingPostingRuleController::class, 'index'])->middleware('permission:accounting.view');
        Route::post('accounting-posting-rules', [AccountingPostingRuleController::class, 'store'])->middleware('permission:accounting.configure');
        Route::put('accounting-posting-rules/{postingRule}', [AccountingPostingRuleController::class, 'update'])->middleware('permission:accounting.configure');
        Route::get('accounting-reports/trial-balance', [AccountingReportController::class, 'trialBalance'])->middleware('permission:reports.view');
        Route::get('accounting-reports/income-statement', [AccountingReportController::class, 'incomeStatement'])->middleware('permission:reports.view');
        Route::get('accounting-reports/balance-sheet', [AccountingReportController::class, 'balanceSheet'])->middleware('permission:reports.view');
        Route::get('accounting-reports/subsidiary-ledger', [AccountingReportController::class, 'subsidiary'])->middleware('permission:reports.view');
        Route::get('cooperative-bank-accounts', [BankReconciliationController::class, 'accounts'])->middleware('permission:accounting.reconcile');
        Route::post('cooperative-bank-accounts', [BankReconciliationController::class, 'storeAccount'])->middleware('permission:accounting.configure');
        Route::get('bank-reconciliations', [BankReconciliationController::class, 'index'])->middleware('permission:accounting.reconcile');
        Route::post('bank-reconciliations', [BankReconciliationController::class, 'store'])->middleware('permission:accounting.reconcile');
        Route::get('bank-reconciliations/{reconciliation}', [BankReconciliationController::class, 'show'])->middleware('permission:accounting.reconcile');
        Route::post('bank-statement-lines/{statementLine}/match', [BankReconciliationController::class, 'match'])->middleware('permission:accounting.reconcile');
        Route::post('bank-reconciliations/{reconciliation}/auto-match', [BankReconciliationController::class, 'autoMatch'])->middleware('permission:accounting.reconcile');
        Route::patch('bank-reconciliations/{reconciliation}/complete', [BankReconciliationController::class, 'complete'])->middleware('permission:accounting.reconcile');
    });
});
